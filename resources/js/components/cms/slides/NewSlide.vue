  ...
  <form class="edit-page-info form new-slide-form" @submit.prevent="createSlide()" aria-label="Create a new slide">
    <fieldset class="form-inner">
        ...
        <div v-if="showImageGrid" id="image-grid" class="image-grid" role="region" aria-label="Image selection">
            ...
            <button @click.prevent="showNewImage = !showNewImage" :class="showNewImage ? 'toggle-new-img new-image-open cancel-new-image' : 'toggle-new-img'" aria-label="New image"><font-awesome-icon :icon="['fas', 'plus']" /></button>
            <NewImage v-if="showNewImage" @refreshImages="getImages" />
            <button class="cancel-new-image new-layout" v-if="showNewImage" @click.prevent="showNewImage = false">Cancel New Image</button>
        </div>
