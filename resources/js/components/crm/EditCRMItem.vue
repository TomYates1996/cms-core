<template>
    <form class="new-crm-item form" @submit.prevent="updateItem()" aria-label="Create New Listing">
        ...
        <div class="form-section media-section">
            ...
            <div class="form-field">
                ...
                <ul>
                    ...
                    {{ file.name }}
                    <img class="media-gallery-img" :src="'/' + file" alt="img">
            ...
            <img class="thumbnail-image-preview" :src="imagePreview" alt="Thumbnail Image">
        </div>
        ...
        <div v-if="isEvent" class="form-section openings-section">
            ...
            <ul v-if="form.start_datetime && oneDay" class="day-list">
                <div v-for="(day, index) in calculatedDaysList" class="day-input-wrap" :key="index">
                    ...
                    <div v-if="!day.all_day || day.all_day === '0'" class="times-section">
            ...
            <ul v-if="form.start_datetime && !oneDay" class="day-list">
                <div v-for="(day, index) in daysList" class="day-input-wrap" :key="index">
                    ...
                    <div v-if="!day.all_day || day.all_day === '0'" class="times-section">
                        ...
                        <button type="button" @click="day.times.push({ from: '', to: '' })" v-if="!day.all_day || day.all_day === '0'" class="add-time">Add Time</button>
                    </div>
                </div>
            </ul>    
        </div>
...
export default {
  ...
  setup(props) {
    const form = useForm({
      ...
      slug: props.existingItem.slug || '',
      category_id: props.existingItem.category_id || null,
      sub_category_id: props.existingItem.sub_category_id || null,
      short_description: props.existingItem.short_description || '',
      description: props.existingItem.description || '',
      tags: props.existingItem.tags?.join(', ') || '',
      address: props.existingItem.address || '',
      city: props.existingItem.city || '',
      region: props.existingItem.region || '',
      country: props.existingItem.country || '',
      postcode: props.existingItem.postcode || '',
      latitude: props.existingItem.latitude || null,
      longitude: props.existingItem.longitude || null,
      phone_number: props.existingItem.phone_number || '',
      email: props.existingItem.email || '',
      website: props.existingItem.website || '',
      media_gallery: [],
      thumbnail_image: '',
      opening_hours: props.existingItem.opening_hours || [],
      prices: props.existingItem.prices || [],
      booking_url: props.existingItem.booking_url || '',
      reservation_email: props.existingItem.reservation_email || '',
      featured: props.existingItem.featured || false,
      owner_id: props.existingItem.owner_id || null,
      published_at: props.existingItem.published_at || null,
      social_links: props.existingItem.social_links || [],
      amenities: props.existingItem.amenities?.map(name => ({ name, checked: true })) || [],
      accessibility_info: props.existingItem.accessibility_info || {},
      listing_ids: props.existingItem.listing_ids || [],
      event_ids: props.existingItem.event_ids || [],
      organiser_name: props.existingItem.organiser_name || '',
      organiser_email: props.existingItem.organiser_email || '',
      organiser_phone: props.existingItem.organiser_phone || '',
      organiser_id: props.existingItem.organiser_id || '',
      openings: props.existingItem.openings || [],
      start_datetime: props.existingItem.start_datetime || '',
      end_datetime: props.existingItem.end_datetime || '',
      recurrence: props.existingItem.recurrence || 'None',
    });
    ...
    created() {
        ...
        this.form.start_datetime = this.form.start_datetime?.split(' ')[0] || '';
        this.form.end_datetime = this.form.end_datetime?.split(' ')[0] || '';
    },
    computed: {
...
generateOpenings() {
    ...
    updateItem() {
        ...
        const updateRoute = this.isEvent
            ? route('api.event.update', this.existingItem.id)
            : route('api.listing.update', this.existingItem.id);

        this.form.post(updateRoute, {
            preserveScroll: true,
            onSuccess: () => {
            const redirectPath = this.isEvent ? '/cms/crm/events' : '/cms/crm/listings';
            this.$inertia.visit(redirectPath);
            },
            onError: (errors) => {
    ...
    calculateDays() {
        ...
        const sourceMap = Object.fromEntries(this.form.openings.map(item => [item.name, item]));

        this.daysList = this.daysList.map(item => sourceMap[item.name] || item);
    ...
    mounted() {
        if (this.existingItem.thumbnail_image) {
            this.imagePreview = '/' + this.existingItem.thumbnail_image;
        }

        if (Array.isArray(this.existingItem.media_gallery)) {
            this.form.media_gallery = this.existingItem.media_gallery;
        }
