<template>
  <form class="edit-page-info form" @submit.prevent="createCategory()" aria-label="Create New Category">
    <fieldset class="form-inner">
      <legend class="form-title">Create New Subcategory for {{ forText }}</legend>
...
export default {
    ...
    methods: {
        createCategory () {
            this.form.parent_cat_id = this.parentId;

            this.form.post(route('api.subcategory.store'), {
                onSuccess: () => {
                    const newSub = {
                        id: Date.now(), 
                        name: this.form.name,
                    ...
                    this.$emit('subcategory-created', newSub);
                },
                onError: (errors) => {
